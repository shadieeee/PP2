...
cooldown_start = 0
COOLDOWN_TIME = 5000 # 5 seconds in milliseconds
...
class Food:
    ...
    def __init__(self):
        self.colors = [colorGREEN, colorBLUE,colorRED]
    ...
    def draw(self):
        self.cooldown_start = pygame.time.get_ticks()
        pygame.draw.rect(screen, self.colors[self.n], (self.pos.x * CELL, self.pos.y * CELL, CELL, CELL))
...
Level: {level}"""
    ...
    snake.draw()
    if abs(current_time - food.cooldown_start) > COOLDOWN_TIME:
